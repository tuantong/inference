...
def download_images_in_parallel_and_save(image_data, folder_path, max_workers=10):
    ...
    if max_workers > 1:
        pool_data = [
            (url, folder_path, filename) for filename, url in image_data.items()
        ]
        with multiprocessing.Pool(processes=max_workers) as pool, tqdm(
            ...
            for async_result in async_results:
                try:
                    ...
                    logger.error(
                        f"Download for item ID {item_id} timed out. Skipping..."
                    )
                pbar.update()
    else:
        for filename, url in tqdm(
            image_data.items(),
            desc="Downloading images",
            disable=True,
            total=len(image_data),
        ):
            args = (url, folder_path, filename)
            item_id, image_path = download_and_save_image(args)
            results[item_id] = image_path
...
def find_similar_set_for_new_variants_with_image(
    ...
    """
    verbose = kwargs.get("verbose", False)
    max_workers = kwargs.get("max_workers", 0)
